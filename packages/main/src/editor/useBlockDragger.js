...
export function useBlockDragger(focusData, lastSelectBlock, data) {
  ...
  const mousemove = (e) => {
    ...
    focusData.value.focus.forEach((block, idx) => {
      ...
      if (block.left > container_width - block.width)
        block.left = container_width - block.width;
      if (block.top < 0) block.top = 0;
      if (block.top > container_height - block.height)
        block.top = container_height - block.height;
    });
  };
  const mouseup = (e) => {
